...
//        System.out.println("Terminate: " + Arrays.toString(((ArrayWritable) buffer).get()));
    ...
    public static void main(String[] args) {
        ...
        Writable aw1 = new ArrayWritable(Text.class);
        Writable[] w1 = ((ArrayWritable) aw1).get();
        if(w1==null){
            w1 = new Writable[10];
        }
        String s1 = "12aa";
...
//        Text t2 = new Text(s2);
        ...
        ((Text)w1[0]).set(t1.getBytes());

        byte[] bytes = new byte[s2.getBytes().length];
        ((Text) w1[0]).set(bytes);
        System.arraycopy(s2.getBytes(), 0,((Text) w1[0]).getBytes() , 0, s2.getBytes().length);

//        w1[0] = new Text(bytes);
        System.out.println(Arrays.toString(((Text) w1[0]).getBytes()));
